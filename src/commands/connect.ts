...
export default class Connect extends Command {
  ...
  static override examples = [
    "<%= config.bin %> <%= command.id %> --u my-username --s my-secret",
  ];

  static flags = {
    secret: Flags.string({
      char: "s",
      description: "the secret to connect to the cloud",
    }),
    username: Flags.string({
      char: "u",
      description: "the username to connect to the cloud",
    }),
  };
  ...
  public async run(): Promise<void> {
    ...
    const dashboardLink = terminalLink(
      "nexuscraft.",
      "https://nexuscraft.io/dashboard"
    );
    ...
    if (!usernameValue) {
      this.error(
        chalk.red(
          "No username provided. Please provide a username to connect to the cloud."
        )
      );
    }
    ...
    if (!secretValue) {
      this.error(
        chalk.red(
          "No secret provided. Please provide a secret to connect to the cloud."
        )
      );
    }
